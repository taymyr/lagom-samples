...
lazy val `jpa-crud` = (project in file("."))
  .aggregate(`jpa-crud-api`, `jpa-crud-impl`)

lazy val `jpa-crud-api` = (project in file("api"))
  .settings(javaCompileSettings: _*)
  .settings(libraryDependencies ++= Seq(lagomJavadslApi, lombok))

lazy val `jpa-crud-impl` = (project in file("impl"))
...
lazy val hibernateJpaModelGenSettings = Seq(managedClasspath in Compile := {
